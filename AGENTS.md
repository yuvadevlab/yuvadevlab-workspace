...
1. **SSO Authentication Flow**: Frontend apps authenticate via `@yuvadevlab/auth-client` against `sso`. JWTs are validated at `api-gateway` using public keys before routing to downstream microservices.
2. **Order-to-Delivery Event Flow**: Asynchronous state changes propagate through Kafka using `@yuvadevlab/kafka-client` (`OrderCreated` -> `InventoryReserved` -> `PaymentProcessed` -> `NotificationSent`).
3. **AI RAG Pipeline**: API Gateway routes chat queries -> NestJS validates tenant DB context -> Python worker embeds query and executes hybrid search in PostgreSQL (`pgvector` + `pg_trgm`) -> Ollama LLM streams responses back via SSE.
4. **Resilience & Circuit Breaker**: API Gateway uses Opossum circuit breakers. 5 consecutive failures trip the circuit to OPEN, returning fast 503 responses to protect downstream services.
...
# Example: pnpm nx run web-super-app:dev
